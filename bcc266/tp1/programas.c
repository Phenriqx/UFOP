...
int programaSomaDivisores(CPU *cpu, RAM *ram, int posInicialRAM, int n);
...
int programaModulo(CPU* cpu, RAM *ram, int posInicial, int a, int b) {
  //RAM *ram = criarRAM_vazia(3);
  int posA = posInicial;
  int posB = posInicial + 1;
  int posMult = posInicial + 2;

  int bufferMult = posInicial + 10;
  int bufferDiv = posInicial + 20;

  Instruction **trecho1 = (Instruction **)malloc(5 * sizeof(Instruction *));
  trecho1[0] = setInstruction(1, a, -1, 4); //reg1 <- a
  trecho1[1] = setInstruction(1, posA, -1, 5); //ram[posA] <- reg1
  trecho1[2] = setInstruction(2, b, -1, 4); //reg2 <- b
  trecho1[3] = setInstruction(2, posB, -1, 5); //ram[posB] <- reg2
  trecho1[4] = setInstruction(-1, -1, -1, -1);
  ...
  int div = programaDiv(cpu, ram, bufferDiv, getDado(posA, ram), getDado(posB, ram)); // divisão entre a e b

  int mult = programaMulti(cpu, ram, bufferMult, div, getDado(posB, ram));

  Instruction **trecho2 = (Instruction **)malloc(4 * sizeof(Instruction *));
  trecho2[0] = setInstruction(1, mult, -1, 4); //reg1 <- mult
  trecho2[1] = setInstruction(1, posMult, -1, 5); //ram[posMult] <- reg1
  trecho2[2] = setInstruction(posA, posMult, posA, 1); //ram[posA] <- ram[posA] - ram[posMult]
  ...
  int result = getDado(posA, ram);

  trecho2 = destroiTrecho(trecho2, 4);
  ram = zerarRAM(ram, posInicial, posB);
...
int programaSomaDivisores(CPU *cpu, RAM *ram, int posInicialRAM, int n) {
  int posSomaRAM = posInicialRAM + 10;       
  int posDivisorRAM = posInicialRAM + 11; 

  Instruction **trechoInit = (Instruction **) malloc(3 * sizeof(Instruction *));
  trechoInit[0] = setInstruction(1, 1, -1, 4); // reg1 <- 1
  trechoInit[1] = setInstruction(1, posSomaRAM, -1, 5); // ram[posSoma] <- reg1
  trechoInit[2] = setInstruction(-1, -1, -1, -1);

  setPrograma(cpu, trechoInit, 3);
  iniciar(cpu, ram);
  destroiPrograma(cpu, 3);
  trechoInit = destroiTrecho(trechoInit, 3);

  if (n <= 1)
    return 0;

  Instruction **trechoSoma = (Instruction **) malloc(2 * sizeof(Instruction *));
  trechoSoma[0] = setInstruction(posSomaRAM, posDivisorRAM, posSomaRAM, 0);
  trechoSoma[1] = setInstruction(-1, -1, -1, -1);


  for (int i = 2; i < n; i++) {
    int resto = programaModulo(cpu, ram, posInicialRAM, n, i);

    if (resto == 0) {
      setDado(posDivisorRAM, i, ram);

      setPrograma(cpu, trechoSoma, 2);
      iniciar(cpu, ram);
      destroiPrograma(cpu, 2);
    }
  }

  trechoSoma = destroiTrecho(trechoSoma, 2);

  return getDado(posSomaRAM, ram);
}

bool programaEhPerfeito(CPU *cpu, RAM *ram, int posInicialRAM, int n) {
  if (n <= 1)
    return 0;
  
  int soma_divisores = programaSomaDivisores(cpu, ram, posInicialRAM + 10, n);
  printf("chefa aqui?\n");

  int posN = posInicialRAM; 
  int posSoma = posInicialRAM + 1; 

  Instruction **trecho = (Instruction **)malloc(6 * sizeof(Instruction *));
  trecho[0] = setInstruction(1, n, -1, 4); // reg1 <- n
  trecho[1] = setInstruction(1, posN, -1, 5); // ram[posN] <- reg1
  trecho[2] = setInstruction(2, soma_divisores, -1, 4); // reg2 <- soma_divisores
  trecho[3] = setInstruction(2, posSoma, -1, 5); // ram[posSoma] <- reg2
  trecho[4] = setInstruction(posN, posSoma, posN, 1); //ram[posN] <- ram[posN] - ram[posSoma]
